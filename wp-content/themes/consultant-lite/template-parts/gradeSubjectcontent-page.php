	...
	<div class="entry-content">
		...
		<!------------ DATATABLES --------------->
			   <table id='myTable' class='display' border="0" style="font-size: 15px;">
				 ...
				 <tbody>
				   <?php
						$result = $wpdb->get_results("SELECT * FROM gradelevel_subject_tbl WHERE deleted = 0");
			...
			<div style="padding-top: 30px; padding-left: 35px; background-color: #E5E7E9 ;">
				...
				<table id="tblinfo" border="0" style="font-size: 1.5rem; text-align: left;">
				 ...
				 <tr>
				   ...
				   <td>Subject:</td>
				   <td><input type="text" id="gsubject" name="gsubject" size="10" /></td>			   
				 </tr>				 
				</table> 
...
$(document).ready(function(){	
	...
	$('#btnSave').click(function(){
       ...
       $.post("http://localhost/SJA-grading/wp-content/themes/consultant-lite/gradeSubjectajax.php",
		{
		  action: action,
		  gsID: gsID, 
		  grdlvl: grdlvl,
  ...
  function delgs(gsId){
     ...
     $.post("http://localhost/SJA-grading/wp-content/themes/consultant-lite/gradeSubjectajax.php",
		...
		function(data, status){	
		  var gs = data.split("|");
		  $('#tblinfo').css("visibility", "hidden");
          $('#record_del').attr("style","display: block; font-size: 1.5rem;");          
          $('#record_del').html('Are you sure you want to delete grade '+gs[0]+' - '+gs[1]+'?');		  
          $('.hover_bkgr_fricc').show();
